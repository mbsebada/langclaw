...
ZALO_SERVICE_API_KEY = os.environ.get("ZALO_SERVICE_API_KEY", "dev-zalo-key-123")
...
async def _proxy_to_zalo(
    ...
    headers = {"x-api-key": ZALO_SERVICE_API_KEY}
...
async def send_outreach_message(
    ...
    try:
        if ZALO_PHONE_OVERRIDE:
            logger.info(f"Using ZALO_PHONE_OVERRIDE: {ZALO_PHONE_OVERRIDE}")
            phone = ZALO_PHONE_OVERRIDE
